...
public:
    ...
    {
        const auto body_model = m_World.create();

        kawe::Mesh::emplace(m_World, body_model, model_path);

        // loading texture if one is provided
        if (!texture_path.empty())
            kawe::Texture2D::emplace(m_World, body_model, "./asset/textures/Planet_4K.jpg");
    ...
    auto on_update_bodies(const kawe::TimeElapsed &e) -> void
    {
        ...
        for (const auto &body : m_Bodies)
            for (const auto &other : m_Bodies) {
                ...
                const auto force = force_dir * GRAVITATIONAL_CONSTANT * static_cast<double>(body_mass)
                                   * static_cast<double>(other_mass) / static_cast<double>(sqr_dist);
